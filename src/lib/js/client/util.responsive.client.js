...
import { anyOpenLeftMenu, closeAllLeftMenus } from "./util.left.menus.client";
import { waitFor } from "../common/util.common";
// import { anyOpenModal, closeAllModals } from "./util.modals.client.common";
...
function setupTouchable() {
    const mqlTouchable = window.matchMedia(
        "((hover: none) and (pointer: fine)) or ((hover: none) and (pointer: coarse)) or ((hover: hover) and (pointer: coarse))"
    );
...
export function onMountTouchable(param) {
    ...
    if (get(touchable)) param.onMount();
}
...
function onHoverable() {
    for (const element of onMountHoverableQueue) element.onMount();
}

function onHoverableReverse() {
    for (const element of onMountHoverableReverseQueue) element.reverse();
}

export function onMountHoverable(param) {
    onMountHoverableQueue.push(param);
    if (get(hoverable)) param.onMount();
}
...
async function onBigScreen() {
    for (const element of onMountSmallScreenReverseQueue) element.reverse();

    for (const element of onMountBigScreenQueue) element.onMount();
...
export function onMountBigScreen(param) {
    ...
    if (get(bigScreen)) param.onMount();
}
...
function onSmallScreen() {
    for (const element of onMountBigScreenReverseQueue) element.reverse();

    for (const element of onMountSmallScreenQueue) element.onMount();
}
...
export function getLinkForResponsiveImage(imageName, media, updatedAt, onceRatio = true) {
    ...
    if (!onceRatio && media.aspectRatio) link += "&ratio=" + media.aspectRatio[0] + "-" + media.aspectRatio[1];
